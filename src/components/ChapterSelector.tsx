...
// This is the same chapter data from TestModal
...
const ChapterSelector: React.FC<ChapterSelectorProps> = ({ selectedChapters, onChange }) => {
  ...
  const toggleAllChaptersForSubject = (subject: string) => {
    const subjectChapters = chapters[subject] || [];
    const allSubjectChapterIds = subjectChapters.map(chapter => chapter.id);
    
    // Check if all chapters of this subject are already selected
    const allSelected = allSubjectChapterIds.every(id => selectedChapters.includes(id));
    
    if (allSelected) {
      // Remove all chapters of this subject
      onChange(selectedChapters.filter(id => !allSubjectChapterIds.includes(id)));
    } else {
      // Add all chapters of this subject that aren't already selected
      const chaptersToAdd = allSubjectChapterIds.filter(id => !selectedChapters.includes(id));
      onChange([...selectedChapters, ...chaptersToAdd]);
    }
  };
  ...
  return (
    <div className="space-y-2 max-h-60 overflow-y-auto">
      {Object.entries(chapters).map(([subject, subjectChapters]) => (
        <div key={subject} className="border rounded-md overflow-hidden">
          <div 
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  toggleAllChaptersForSubject(subject);
                }}
                className="text-xs px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full hover:bg-blue-200"
              >
                {subjectChapters.every(chapter => selectedChapters.includes(chapter.id)) 
                  ? 'Deselect All' 
                  : 'Select All'}
              </button>
            </div>
            {expandedSubjects.includes(subject) ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
          </div>
              ...
              {subjectChapters.map(chapter => (
                ...
                >
                  <div className={`w-4 h-4 border rounded flex items-center justify-center ${
                    selectedChapters.includes(chapter.id) ? 'bg-blue-500 border-blue-500' : 'border-gray-300'
                  }`}>
                    {selectedChapters.includes(chapter.id) && <Check size={12} color="white" />}
                  </div>
